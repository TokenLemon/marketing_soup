...
export default function StakeholderView({
  ...
  async function runAISearch() {
    ...
    try {
      ...
      setError('Error finding stakeholders. Try manual entry.')
    }
    setLoading(false)
  }
  ...
  const TAB_STYLE = (active: boolean, color?: string) => ({
    padding: '8px 16px',
    borderRadius: '8px',
    border: `1px solid ${active ? (color || '#0066cc') : '#e8e8ed'}`,
    background: active ? (color ? color + '15' : '#e8f4ff') : '#ffffff',
    color: active ? (color || '#0066cc') : '#444460',
    fontSize: '12px',
    ...
    <div style={{ background: '#ffffff', border: '1px solid #e8e8ed', borderRadius: '12px', overflow: 'hidden', marginBottom: '16px' }}>
      <div style={{ padding: '14px 18px', borderBottom: '1px solid #e8e8ed', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <span style={{ fontSize: '10px', background: '#e8f4ff', color: '#0066cc', padding: '2px 8px', borderRadius: '20px', fontWeight: 600 }}>
          {stakeholders.length > 0 ? `${stakeholders.length} FOUND` : 'SELECT SOURCE'}
        </span>
      </div>
      ...
      <div style={{ padding: '18px' }}>
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            <button
              ...
              style={TAB_STYLE(mode === 'ai', '#0066cc')}
            >
              ✦ AI Web Research
            </button>
            <button
              onClick={() => { setMode('notion'); setStakeholders([]); setSelected(null) }}
              style={TAB_STYLE(mode === 'notion', '#6366f1')}
            >
              N  Import from Notion
            </button>
            <button
              onClick={() => { setMode('manual'); }}
              style={TAB_STYLE(mode === 'manual', '#059669')}
            >
              + Add Manually
            </button>
          </div>
        ...
        {mode === 'ai' && (
          <div>
            {loading && (
              <div style={{ padding: '20px', textAlign: 'center', color: '#8888a0', fontSize: '13px' }}>
                <div style={{ marginBottom: '8px', fontSize: '20px' }}>⟳</div>
                Searching for real stakeholders at {company}...
              </div>
            )}
            {error && (
            ...
            {!loading && stakeholders.length === 0 && !error && (
              <button onClick={runAISearch}
                style={{ background: '#0066cc', color: '#fff', border: 'none', borderRadius: '8px', padding: '10px 18px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', marginBottom: '12px' }}>
                Search for stakeholders at {company} →
              </button>
            )}
            {!loading && stakeholders.length > 0 && (
              <div style={{ marginBottom: '8px', fontSize: '12px', color: '#059669', fontWeight: 500 }}>
                ✓ Found {stakeholders.length} stakeholders via AI research
              </div>
            )}
          </div>
        )}
        ...
        {mode === 'notion' && (
          <div style={{ background: '#f5f5f7', border: '1px solid #e8e8ed', borderRadius: '8px', padding: '20px', marginBottom: '12px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '12px', color: '#8888a0', marginBottom: '14px', lineHeight: '1.6' }}>
              Connect your Notion workspace in Settings to pull real contacts from your CRM database automatically.
            </div>
